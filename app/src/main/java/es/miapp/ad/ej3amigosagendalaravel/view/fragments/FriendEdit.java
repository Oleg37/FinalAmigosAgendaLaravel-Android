...
import es.miapp.ad.ej3amigosagendalaravel.databinding.FragmentFriendEditBinding;
...
public class FriendEdit extends Fragment implements CalendarView.OnDateChangeListener {
    ...
    private void init() {
        if (friend.getDateOfBirth() != null) {
            b.cvDOFBirth.setDate(DateTransform.dateJava(friend.getDateOfBirth()));
            date = DateTransform.dateJava(friend.getDateOfBirth());
        }
    ...
    private void verificarDatos() {
        ...
        if (validado) {
            ...
            friend.setDateOfBirth(DateTransform.dateSQLYYYYMMDD(date));

            viewModel.updateFriend(friend.getId(), friend);
